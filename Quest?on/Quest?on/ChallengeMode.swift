...
class ChallengeMode: SKScene {
    ...
    var start: Date = Date()
    var end: Date = Date()
    var timeOfPlay: Int = 0
    var shortedtTimeOfAns: Int = 180
    var maxTime = 180
    var counter = 0
    var timer: Timer?
    ...
    func checkUserAnswer(_ area: Int){
        ...
        if userAns == answer{
            numOfCorrectAns += 1
            //let now = Date()
            let t = maxTime - counter
            if(t < shortedtTimeOfAns){
                shortedtTimeOfAns = t   // update shortest record
            }
            if(numOfCorrectAns % 10 == 0){
                if(maxTime > 10){
                    maxTime -= 10
                }
            }
    ...
    func presentTimer(){
        counter = maxTime
        let timerNode = SKLabelNode(text: secondFormat(counter))
        timerNode.name = "timer"
        timerNode.position = CGPoint(x: self.frame.midX, y: (self.childNode(withName: "question")?.position.y)! - 50)
    ...
    @objc func updateCounter(){
        counter -= 1
        if(counter > 0){
            if let node = self.childNode(withName: "timer") as? SKLabelNode{
                node.text = secondFormat(counter)
            }
        }else if counter == 0{
            gameOver()
        }
    ...
    func gameOver(){
        timer?.invalidate()
        // show result
        let resultBG = SKSpriteNode(imageNamed: "Result_"+styleList[style])
        resultBG.position = CGPoint(x: self.frame.midX, y: self.frame.midY * 1.5)
        resultBG.zPosition = 1
        
        let resultQ = SKLabelNode(text: String(numOfQuestion))
        resultQ.position = CGPoint(x: self.frame.midX * 1.1, y: self.frame.midY * 1.558)
        resultQ.fontColor = UIColor(red: 0.38, green: 0.33, blue: 0.27, alpha: 1.00)
        resultQ.fontSize = 30
        resultQ.zPosition = 2
        let resultC = SKLabelNode(text: String(numOfCorrectAns))
        resultC.position = CGPoint(x: self.frame.midX * 1.1, y: resultQ.position.y - 35)
        resultC.fontColor = UIColor(red: 0.38, green: 0.33, blue: 0.27, alpha: 1.00)
        resultC.fontSize = 30
        resultC.zPosition = 2
        
        end = Date()
        timeOfPlay = Calendar.current.dateComponents([.second], from: start, to: end).second!
        //print(timeOfPlay)
        print("total:\(numOfQuestion)")
        print("correct:\(numOfCorrectAns)")
        print("time:\(timeOfPlay)")
        
        let resultT = SKLabelNode(text: secondFormat(timeOfPlay))
        resultT.position = CGPoint(x: self.frame.midX * 1.1, y: resultC.position.y - 35)
        resultT.fontColor = UIColor(red: 0.38, green: 0.33, blue: 0.27, alpha: 1.00)
        resultT.fontSize = 30
        resultT.zPosition = 2
        let finishNode = SKLabelNode(text: "Touch here to finish")
        finishNode.name = "finish"
        finishNode.position = CGPoint(x: self.frame.midX, y: resultT.position.y - 50)
        finishNode.fontColor = UIColor(red: 0.38, green: 0.33, blue: 0.27, alpha: 1.00)
        finishNode.fontSize = 35
        finishNode.zPosition = 2
        
        self.addChild(resultBG)
        self.addChild(resultQ)
        self.addChild(resultC)
        self.addChild(resultT)
        self.addChild(finishNode)
    }
    
    func writeRecord(){
        if(result == false || counter == 0){    // game over
            end = Date()
            timeOfPlay = Calendar.current.dateComponents([.second], from: start, to: end).second!
        }
        let totalNumOfQuestion = record.object(forKey: "totalNumOfChallengeQuestion") as! Int?
        if totalNumOfQuestion == nil{   // 累計答題數
            record.set(numOfQuestion, forKey: "totalNumOfChallengeQuestion")
        }else{
            record.set(numOfQuestion + totalNumOfQuestion!, forKey: "totalNumOfChallengeQuestion")
        }
        let totalNumOfAns = record.object(forKey: "totalNumOfChallengeCorrectAns") as! Int?
        if totalNumOfAns == nil{    //  累計答對題數
            record.set(numOfCorrectAns, forKey: "totalNumOfChallengeCorrectAns")
        }else{
            record.set(numOfCorrectAns + totalNumOfAns!, forKey: "totalNumOfChallengeCorrectAns")
        }
        let largestNumOfClearedQuestion = record.object(forKey: "largestNumOfChallengeClearedQuestion") as! Int?
        if largestNumOfClearedQuestion == nil { //  最長通關紀錄
            record.set(numOfCorrectAns, forKey: "largestNumOfChallengeClearedQuestion")
            record.set(timeOfPlay, forKey: "timeOfChallengeCleared")
        }else{
            if largestNumOfClearedQuestion! < numOfCorrectAns {
                record.set(numOfCorrectAns, forKey: "largestNumOfChallengeClearedQuestion")
                //  最短通關時間
                record.set(timeOfPlay, forKey: "timeOfChallengeCleared")
            }else if largestNumOfClearedQuestion! == numOfCorrectAns {
                let timeOfrecord = record.object(forKey: "timeOfChallengeCleared") as! Int?
                if timeOfPlay < timeOfrecord! {
                    record.set(numOfCorrectAns, forKey: "largestNumOfChallengeClearedQuestion")
                    record.set(timeOfPlay, forKey: "timeOfChallengeCleared")
                }
            }
        }
        let fastestRecord = record.object(forKey: "shortestTimeOfChallengeClear") as! Int?
        if fastestRecord == nil {   //  最短答題時間
            record.set(shortedtTimeOfAns, forKey: "shortestTimeOfChallengeClear")
        }else if shortedtTimeOfAns < fastestRecord! {
            record.set(shortedtTimeOfAns, forKey: "shortestTimeOfChallengeClear")
        }
        record.synchronize()
    }
